...
var startSplitMarkerOffset;
var splitMarkerOffset;
var splitMarkerTextOffset;
...
function setup() {
    ...
    runsNumber = ["Run 1 of 4", "Run 2 of 4", "Run 3 of 4", "Run 4 of 4"]
    ...
    startSplitMarkerOffset = 4
    splitMarkerOffset = 7
    splitMarkerTextOffset = -10
}
...
function draw() {
    ...
    // potential add descriptive text
    // scale(1,1)
    ...
    for(i = 0; i < splitMarkers.length; i++) {
        quad(splitMarkers[i] + 8 * laneOffset,  startHeight,
            ...
            splitMarkers[i] + splitLineThickness*perspectiveFactor*1.5, startHeight + 8 * rowHeight,
            splitMarkers[i] + splitLineThickness*perspectiveFactor*0.5, startHeight + 8 * rowHeight)
    }
    for(i = 0; i < splitMarkers.length; i++) {
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 8; j++) {
            textSize(rowHeight / 3 + j/1.5);
            text(names[j], ends[i] + 100 - j * laneOffset - 75 - j, startHeight + j * rowHeight + rowHeight * 2 / 3)
        }
    }
    ...
    for (i = 0; i < 4; i++) {
        ...
        push();
            ...
            quad(50,                    1.2 * rowHeight,
                147,                    1.2 * rowHeight,
                147 - laneOffset*0.3,   1.5 * rowHeight,
                50 - laneOffset*0.3,    1.5 * rowHeight)
            textSize(80)
            ...
            quad(60,      1.6 * rowHeight,
                69,      1.6 * rowHeight,
                57 - laneOffset*0.3,  3.4 * rowHeight,
                47 - laneOffset*0.3,  3.4 * rowHeight)
            textSize(80)
            ...
            quad(32,      3.5 * rowHeight,
                129,      3.5 * rowHeight,
                129 - laneOffset*0.3,  3.85 * rowHeight,
                32 - laneOffset*0.3,  3.85 * rowHeight)
            textSize(80)
            ...
            quad(125,      1.6 * rowHeight,
                134,      1.6 * rowHeight,
                122 - laneOffset*0.3,  3.4 * rowHeight,
                112 - laneOffset*0.3,  3.4 * rowHeight)
            textSize(85)
    ...
    for (i = 0; i < 4; i++) {
        push();
            ...
            push();
                ...
                fill(color('rgba(245,245,245, 0.8)'));
            ...
            push();
                ...
                textAlign(CENTER);
                textSize(17)
                fill(color('rgba(62,118,236, 0.4)'));
                text("o", flagWidthCountry*0.18, flagWidthCountry/1.9)
                fill(color('rgba(0,0,0, 0.4)'));
                text("o", flagWidthCountry*0.47, flagWidthCountry/1.9)
                fill(color('rgba(255,0,0, 0.4)'));
                text("o", flagWidthCountry*0.76, flagWidthCountry/1.9)
                textSize(18)
                fill(color('rgba(255,206,1, 0.6)'));
                text("o", flagWidthCountry*0.28, flagWidthCountry/1.5)
                fill(color('rgba(23,154,19, 0.4)'));
                text("o", flagWidthCountry*0.58, flagWidthCountry/1.5)
            pop();
        pop();
    }
    ...
    for (i = 0; i < 4; i++) {
        push();
            translate(ends[i] + 40, startHeight + 0.14*rowHeight)
            shearX(-17.0);
    ...
    //////////////////// athlete names first, second, third place athlete names and flags at end of run ////////////////////
    textFont("Noto Sans");
    textStyle(ITALIC);
    textAlign(LEFT);
    fill(color('rgba(38, 21, 255, 0.25)'));
    ...
    // at end of run 1
    var e = 0
    if(frameCount >= endLineFlash[e]) {
        textAlign(RIGHT);
        var l = 0
        textSize(rowHeight / 3 + l/1.5);
        fill(color('rgba(218, 165, 32, 0.6)')); 
        text(names[l], ends[e] + 100 - l * laneOffset - 75 - l, startHeight + l * rowHeight + rowHeight * 2 / 3)
        push();
            translate(ends[e] + 40, startHeight + 0.14*rowHeight)
            shearX(-17.0);
            ...
            push();
                ...
                quad(0, 0, flagWidthCountry, 0, flagWidthCountry, flagHeightCountry/3, 0, flagHeightCountry/3)
                fill(color('rgba(245,245,245, 0.3)'));
                quad(0, flagHeightCountry/3, flagWidthCountry, flagHeightCountry/3, flagWidthCountry, 2*flagHeightCountry/3, 0, 2*flagHeightCountry/3)
                fill(color('rgba(237,41,57, 0.3)'));
                quad(0, 2*flagHeightCountry/3, flagWidthCountry, 2*flagHeightCountry/3, flagWidthCountry, 3*flagHeightCountry/3, 0, 3*flagHeightCountry/3)
            pop();
    ...
    if(frameCount >= endLineFlash[e]+10) {
        textAlign(RIGHT);
        var l = 4
        textSize(rowHeight / 3 + l/1.5);
    ...
    if(frameCount >= endLineFlash[e]+20) {
        textAlign(RIGHT);
        var l = 2
        textSize(rowHeight / 3 + l/1.5);
    ...
    if(frameCount >= endLineFlash[e]) {
        ...
        text(names[l], ends[e] + 100 - l * laneOffset - 75 - l, startHeight + l * rowHeight + rowHeight * 2 / 3)
        push();
            translate(ends[e] + 40, startHeight + 0.16*rowHeight)
            shearX(-17.0);
            // germany
            push();
                translate(0, 4*rowHeight)
    ...
    if(frameCount >= endLineFlash[e]+20) {
        ...
        push();
            ...
            push();
                ...
                textAlign(CENTER);
                textSize(17)
                fill(color('rgba(62,118,236, 0.3)'));
                text("o", flagWidthCountry*0.18, flagWidthCountry/1.9)
                fill(color('rgba(0,0,0, 0.3)'));
                text("o", flagWidthCountry*0.47, flagWidthCountry/1.9)
                fill(color('rgba(255,0,0, 0.3)'));
                text("o", flagWidthCountry*0.76, flagWidthCountry/1.9)
                textSize(18)
                fill(color('rgba(255,206,1, 0.3)'));
                text("o", flagWidthCountry*0.28, flagWidthCountry/1.5)
                fill(color('rgba(23,154,19, 0.3)'));
                text("o", flagWidthCountry*0.58, flagWidthCountry/1.5)
            pop();
        pop();    
    }
    ...
    if(frameCount >= endLineFlash[e]+10) {
        ...
        push();
            ...
            push();
                ...
                quad(0, 0, flagWidthCountry, 0, flagWidthCountry, flagHeightCountry, 0, flagHeightCountry)
                for(var k = 1; k < 12; k += 2) {
                    fill(color('rgba(245,245,245, 0.2)'));
                    quad(0, flagHeightCountry/13*k, flagWidthCountry, flagHeightCountry/13*k, flagWidthCountry, flagHeightCountry/13*(k+1), 0, flagHeightCountry/13*(k+1))
                }
    ...
    //////////////////// fix wall and track thickness being covered by spray ////////////////////
    // wall
    for (i = 0; i < 4; i++) {
        fill(100);
        noStroke()
        // side
        rect(beginnings[i] + 8 * laneOffset - trackPadding - wallLengthen + courseLength + 2*trackPadding + 2*wallLengthen - 5,         startHeight-farWallHeight+1,          4.5,          nearWallHeight-1)
        // lines
        stroke(255);
        strokeWeight(1);
        line(beginnings[i] + 8 * laneOffset - trackPadding - wallLengthen, startHeight-farWallHeight, ends[i] + 8 * laneOffset + trackPadding + wallLengthen, startHeight-farWallHeight)
    ...
    for(i = 0; i < 4; i++) {
        ...
        fill(color('rgba(187, 229, 228, 1)'));
            beginShape();
                ...
                vertex(ends[i] + 8 * laneOffset + trackPadding, startHeight + trackThickness);
                vertex(ends[i] + trackPadding, startHeight + 8 * rowHeight + trackThickness + 10);
                vertex(ends[i] + trackPadding, startHeight + 8 * rowHeight);
            endShape();
    }
    ...
    for (i = 0; i < 4; i++) {
        ...
        stroke(220);
        strokeWeight(0.3);
        line(beginnings[i] + startSplitMarkerOffset, startHeight + 8 * rowHeight, beginnings[i] + startSplitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth)
        line(beginnings[i] + startSplitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth, beginnings[i] + startSplitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth + 16)
        line(beginnings[i] + splitint1 + splitMarkerOffset, startHeight + 8 * rowHeight, beginnings[i] + splitint1 + splitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth)
        line(beginnings[i] + splitint1 + splitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth, beginnings[i] + splitint1 + splitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth + 16)
        line(beginnings[i] + splitint2 + splitMarkerOffset, startHeight + 8 * rowHeight, beginnings[i] + splitint2 + splitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth)
        line(beginnings[i] + splitint2 + splitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth, beginnings[i] + splitint2 + splitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth + 16)
        line(beginnings[i] + splitint3 + splitMarkerOffset, startHeight + 8 * rowHeight, beginnings[i] + splitint3 + splitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth)
        line(beginnings[i] + splitint3 + splitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth, beginnings[i] + splitint3 + splitMarkerOffset - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth + 16)
        line(beginnings[i] + startSplitMarkerOffset + courseLength, startHeight + 8 * rowHeight, beginnings[i] + startSplitMarkerOffset + courseLength - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth)
        line(beginnings[i] + startSplitMarkerOffset + courseLength - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth, beginnings[i] + startSplitMarkerOffset + courseLength - wallDepth - 1, startHeight + 8 * rowHeight + wallWidth + 16)
        // text
        push();
            scale(1/3.15, 1)
            fill(color('rgba(220, 220, 220, 0.9)'));
            noStroke()
            textFont("Noto Sans");
            textStyle(NORMAL);
            textAlign(LEFT);
            textSize(14)
            // start
            text("START", (beginnings[i] + startSplitMarkerOffset + splitMarkerTextOffset)*3.15, startHeight + 9.1 * rowHeight)
            // intermediate 1
            text("SPLIT 1", (beginnings[i] + splitint1+splitMarkerOffset + splitMarkerTextOffset)*3.15, startHeight + 9.1 * rowHeight)
            // intermediate 2
            text("SPLIT 2", (beginnings[i] + splitint2+splitMarkerOffset + splitMarkerTextOffset)*3.15, startHeight + 9.1 * rowHeight)
            // intermediate 3
            text("SPLIT 3", (beginnings[i] + splitint3+splitMarkerOffset + splitMarkerTextOffset)*3.15, startHeight + 9.1 * rowHeight)
            // Finish
            text("FINISH", (beginnings[i] + startSplitMarkerOffset + splitMarkerTextOffset + courseLength)*3.15, startHeight + 9.1 * rowHeight)
        pop();
        ...
        //     textSize(18)
        //     // copyright
        //     text("julianhlange.com", ends[i]*3.15-100, startHeight + 9.1 * rowHeight)
        // pop();
